...
module dftd4param
   ...
   character(len=2), parameter :: toSymbol(118) = [ &
      & 'H ','He', &
      & 'Li','Be','B ','C ','N ','O ','F ','Ne', &
   ...
   integer,parameter,private :: maxElem = 86
  !> Electronegativity of each species
   real(wp), parameter :: D4chi(maxElem) = reshape([ &
      ...
      & 1.27465977_wp], shape(D4chi))
  !> Chemical hardness of each species
   real(wp),parameter :: D4gam(maxElem) = reshape([ &
      ...
      & 0.10500484_wp], shape(D4gam))
   !> Coordination number dependence of each species
   real(wp),parameter :: D4kcn(maxElem) = reshape([ &
      ...
      & 0.05849285_wp ], shape(D4kcn))
   !> Extension of the charge distribution for each species
   real(wp),parameter :: D4alp(maxElem) = reshape([ &
...
!> in the xtb code the following was implemented via: 
